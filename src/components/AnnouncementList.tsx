...
import PostAnniversaryCard from "./reuseable/PostAnniversaryCard";
    ...
    return (
      <div className="space-y-6 w-full">
        {posts.map((post: any, index: number) => (
          ...
          >
            {post?.type === "announcemnt" || post?.type === "event" ? (
              <PostAnnouncementCard post={post} />
            ) : post?.type === "birthday" || post?.type === "wokanv" ? (
              <PostAnniversaryCard post={post} />
            ) : (
              <PostAnnouncementCard post={post} />
            )}
          </motion.div>
